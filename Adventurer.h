...
class Adventurer : public Character{
    ...
    public:
        Adventurer(const std::string&, const int, const int, double);    ///<Constructor for our Adventurer class. Uses the same inputs as a Character class.
        ...
        void deliverHit(Character* enemy) override;                 ///<The deliverHit() method calls the enemy units sufferDamage() method, and handles the Adventurer units XP gain. It also calls the lvlUp() method when sufficient XP is gained for a levelup.
    };
